...
import { DatabaseMySQL } from './db-server.js'
...
server.get('/get-videos', async (request, reply) => {
  try {
    const results = await db.list(); // ou uma consulta direta no pool
...
server.listen({
  port: 3334,
  host: '0.0.0.0', // Você pode definir o host também (opcional)
}, (err, address) => {
  if (err) {
    console.error(err)
    process.exit(1)
